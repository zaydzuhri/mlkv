...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, has_kv: bool = True):
        super().__init__()
        self.config = config
        self.has_kv = has_kv
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        last_key_value: Optional[Tuple[torch.Tensor]] = None,
        ...
        if self.config.pretraining_tp > 1:
            ...
            if self.has_kv:
                ...
                key_states = last_key_value[0]
                value_states = last_key_value[1]
        else:
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, has_kv: bool = True):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaAttention(config=config, has_kv=has_kv)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList([LlamaDecoderLayer(config, (l in self.key_value_layers)) for l in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # use cache from last layer that had kv head
            kv_i = max([ix for ix, l in enumerate(self.key_value_layers) if l <= idx])
            past_key_value = past_key_values[kv_i] if past_key_values is not None else None
            ...
            # only cache if layer had kv head
            if use_cache and idx in self.key_value_layers:
                last_key_value = layer_outputs[2 if output_attentions else 1]
                next_decoder_cache += (last_key_value,)
